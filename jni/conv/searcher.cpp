        ...
        if (c == character || c == 0xFF) {
			...
			if (aWordloudstrie.IsTerminalNode(_child_node_id)) {
                ...
                for (U32 nMidaPos = 0; nMidaPos < wMidaInfoLength; nMidaPos += uMidaInfoStep) {
                    ...
                    const U08 * pYomiData = aYomiToWordBitArray.Get(wYomiID, wYomiInfoLength, uYomiInfoStep);
                    for (U32 nPos = 0; nPos < wYomiInfoLength; nPos += uYomiInfoStep) {

                        const U16 wCost   = (pYomiData[nPos + 0] << 8) + pYomiData[nPos + 1];
                        const U16 wLeft   = ((pYomiData[nPos + 4] & 0xF0) << 4) + pYomiData[nPos + 2];
                        const U16 wRight  = ((pYomiData[nPos + 4] & 0x0F) << 8) + pYomiData[nPos + 3];
                        const U32 wWordID = (pYomiData[nPos + 7] << 16) + (pYomiData[nPos + 6] << 8) + pYomiData[nPos + 5];
                        ...
                        res.rid     = wRight;
                        res.Word    = std::string((char*)pWord);
                        res.Yomi    = std::string((char*)pYomi);
